...
int main(int argc, char *argv[]) {
    ...
    FILE *file = fopen("obr_novy.jpg","wb");
    if (!file) {
    ...
    while (1) {
        ...
        if (packet.packet_type == DATA) {
            memcpy(&position, packet.payload, sizeof(position));
            to_write = PAYLOAD_SIZE - sizeof(position);

            if (received_bytes_total + to_write > file_size) {
                to_write = file_size - received_bytes_total;
            }
            fseek(file, position, SEEK_SET);

            fwrite(packet.payload + sizeof(position), 1, to_write, file);
