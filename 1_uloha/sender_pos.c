...
#include <unistd.h> 
/*funkcie na prÃ¡cu s operacnym systemom, ako je close (na zatvorenie socketu).*/
...
int main(int argc, char *argv[]) {
    ...
    while (!feof(file)) {
        ...
        memset(packet.payload, 0, PAYLOAD_SIZE);
        memcpy(packet.payload, &position, sizeof(position));
        size_t read_size = fread(packet.payload + sizeof(position), 1, PAYLOAD_SIZE - sizeof(position), file);
        send_packet(sock, &receiver, &packet);
        position += read_size;
    }
